                               ...
                               const int do_optimization)
{
  ...
  if (opt_verbose_level_gt(opts, 1)) {
    ...
    Logger_log(logger, "Creating the base constraints\n");
  }
  ...
  while(lsLength(unprocessed_nodes) > 0) {
    ...
    case TRUEEXP: {
      ...
      if (sbmc_node_info_get_trans_vars(info) == (array_t *)NULL) {
        ...
        if (opt_verbose_level_ge(opts, 6)) {
          Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
          Logger_log(logger, "  [[TRUE]]_%d^0 <=> TRUE: ", i_model);
          Be_DumpSexpr(be_mgr, be_result, errstream);
          Logger_log(logger, "\n");
        }
      }
    ...
    case NOT: {
      ...
      if (sbmc_node_info_get_trans_vars(info) == (array_t *)NULL) {
        ...
        for (d = 0; d <= sbmc_node_info_get_past_depth(info); d++) {
          ...
          if (opt_verbose_level_ge(opts, 6))
            StreamMgr_print_error(streams,  "  [!f]]_%d^%d <=> ![[f]]_%d^%d: ",
                    i_model, d, i_model, d),
              Be_DumpSexpr(be_mgr, be_result, errstream),
              StreamMgr_print_error(streams,  "\n");
        }
    ...
    if (sbmc_node_info_get_aux_F_node(info)) {
      ...
      if (i_model == 0) {
        ...
        if (opt_verbose_level_ge(opts, 6))
          StreamMgr_print_error(streams,  "  <<F f>>_0 <=> FALSE: "),
            Be_DumpSexpr(be_mgr, be_result, errstream),
            StreamMgr_print_error(streams,  "\n");
      }
      else {
    ...
    if ((node_ptr)NULL != sbmc_node_info_get_aux_G_node(info)) {
      ...
      if (i_model == 0) {
        ...
        if (opt_verbose_level_ge(opts, 6))
          StreamMgr_print_error(streams,  "  <<G f>>_0 <=> TRUE: "),
            Be_DumpSexpr(be_mgr, be_result, errstream),
            StreamMgr_print_error(streams,  "\n");
      }
      else {
    ...
    if (((node_ptr)NULL != sbmc_node_info_get_aux_G_node(info))
      ...
      if (opt_verbose_level_ge(opts, 6)) {
        Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
        Logger_log(logger, "  <<F f>>_E <=> <<F f>>_%u: ", i_model);
        Be_DumpSexpr(be_mgr, be_result, errstream);
        Logger_log(logger, "\n");
      }
    }
    ...
    if ((array_t *)NULL != sbmc_node_info_get_trans_vars(info)) {
      ...
      if (opt_verbose_level_ge(opts, 6))
        Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
        Logger_nlog(logger, wffprint, " Doing binding to L for %N\n", node);
      ...
      for (d = 0; d <= sbmc_node_info_get_past_depth(info); d++) {
        ...
        if (opt_verbose_level_ge(opts, 6))
          StreamMgr_print_error(streams,  "  l_%u => ([[f]]_L_%d <=> [[f]]_%u^%d): ",
                  i_model, d, i_model, d),
            Be_DumpSexpr(be_mgr, be_result, errstream),
            StreamMgr_print_error(streams,  "\n");
      }
  ...
  while(lsLength(unprocessed_nodes) > 0) {
    ...
    if ((node_ptr)NULL != sbmc_node_info_get_aux_F_node(info)) {
      ...
      if (i_model == 0) {
        ...
        if (opt_verbose_level_ge(opts, 6)) {
          Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
          Logger_log(logger, "  <<F f>>_0 <=> FALSE: ");
          Be_DumpSexpr(be_mgr, be_result, errstream);
          Logger_log(logger, "\n");
        }
      }
                                     ...
                                     const hash_ptr info_map)
{
  ...
  while(lsLength(unprocessed_nodes) > 0) {
    ...
    if ((node_ptr)NULL != sbmc_node_info_get_aux_F_node(info)) {
      ...
      if (opt_verbose_level_ge(opts, 6)) {
        Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
        Logger_log(logger, "  <<Ff>>_E <=> <<Ff>>_%d: ", k_model);
        Be_DumpSexpr(be_mgr, be_result, errstream);
        Logger_log(logger, "\n");
      }
    }
    ...
    if (sbmc_node_info_get_aux_G_node(info)) {
      ...
      if (opt_verbose_level_ge(opts, 6)) {
        Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
        Logger_log(logger, "  <<Gf>>_E <=> <<Gf>>_%d: ", k_model);
        Be_DumpSexpr(be_mgr, be_result, errstream);
        Logger_log(logger, "\n");
      }
    }
                             ...
                             const hash_ptr info_map)
{
   ...
   */
  {
    ...
    be_constraint = Be_Iff(be_mgr, be_l_kP1, Be_Falsity(be_mgr));
   ...
   */
  {
    ...
    if (opt_verbose_level_gt(opts, 1)) {
      Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
      Logger_log(logger, "Forced (s_E = s_%d)", k);
      if (opt_verbose_level_ge(opts, 6)) {
        Logger_log(logger, ": ");
        Be_DumpSexpr(be_mgr, be_constraint, errstream);
      }
   ...
   */
  {
    ...
    if (opt_verbose_level_gt(opts, 1)) {
      Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
      Logger_log(logger, "Forced (LoopExists <=> InLoop_%d)", k);
      if (opt_verbose_level_ge(opts, 6)) {
        Logger_log(logger, ": ");
        Be_DumpSexpr(be_mgr, be_constraint, errstream);
      }
