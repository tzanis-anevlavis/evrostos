...
This file is part of the ``sbmc'' package of NuSMV version 2.
...
/**Array for caching f(i) values. Since there is only one function
   exported, and those variables are used only within one single call
   of the fun and then freed, they are not a problem for reentrancy
                             ...
                             const int time, const int k, const int l)
{
  ...
  /**Set up cache*/
  count = formulaMap(env, formulatable, ltl_wff, 0);
  bmc_cache_init(opts, count, k, bmc_tab_past_depth);
...
/*!
  \brief Construct f(k) att full pastdepth for the GF-,F-,FG-, or G-operator
...
/*!
  \brief Generate the last f(k) for operators that use the
...
/*!
  \brief Generate f(k,pastdepth) when pastdepth is less than
...
/*!
  \brief Map temporal subformulas to an integer, returns the
           ...
           const node_ptr ltl_wff, unsigned TLcount)
{
  ...
  Bmc_Stack_ptr thestack = Bmc_Stack_new_stack(env);
...
/*!
  \brief Genrates a boolean expression which is true iff the ltl
...
/*!
  \brief Generates a boolean expression which is true iff the ltl
...
/*!
  \brief Returns a pointer to the el(time) variable
...
/*!
  \brief Creates the expression: \wedge_{i=0}^{k-1} el_i =>
                      (s_i <=> s_k)
...
/*!
  \brief Returns a pointer to the il(time) variable
...
get_il_at_time(const BeEnc_ptr be_enc, const int time, const int k)
{
  ...
  /**check cache*/
  if (bmc_cache_fetch_il(opts, time, k) != (be_ptr)NULL) {
    return bmc_cache_fetch_il(opts, time, k);
  }

  if (time == 1) {
    return bmc_cache_insert_il(opts, time, k, get_el_at_time(be_enc, 0, k));
  }
...
/*!
  \brief Returns a pointer to the le variable
...
/*!
  \brief Initialises the chache used to store f_i(time) and g_(time)
                      values.
