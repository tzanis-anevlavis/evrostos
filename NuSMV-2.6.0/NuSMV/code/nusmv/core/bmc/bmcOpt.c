...
static void* opt_get_integer(OptsHandler_ptr opts,
                             const char* val, void* arg);

static void* opt_get_string(OptsHandler_ptr opts,
                            const char* val, void* arg);

static boolean opt_check_bmc_pb_length(OptsHandler_ptr opts,
                                       const char* val, void* arg);
static boolean opt_check_bmc_pb_loop(OptsHandler_ptr opts,
                                     const char* val, void* arg);
static boolean opt_check_bmc_invar_alg(OptsHandler_ptr opts,
                                       const char* val, void* arg);
static void* opt_get_bmc_invar_alg(OptsHandler_ptr opts,
                                   const char* val, void* arg);
...
static boolean opt_check_bmc_inc_invar_alg(OptsHandler_ptr opts,
                                           const char* val,
                                           void* arg);
static void* opt_get_bmc_inc_invar_alg(OptsHandler_ptr opts,
                                       const char* val, void* arg);
#endif
                                       ...
                                       const char* val, void* arg)
{
  ...
  char* str_loop =
    OptsHandler_get_string_option_value(opts,
                                        BMC_PB_LOOP);
  ...
  if ((res == OUTCOME_SUCCESS) && 
    ...
    StreamMgr_print_error(streams,  "Given value is not compatible with current ");
                                     ...
                                     const char* val, void* arg)
{
  ...
  StreamMgr_ptr streams = STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  Outcome res;
  int length = OptsHandler_get_int_option_value(opts,
                                                BMC_PB_LENGTH);
  ...
  if ((res == OUTCOME_SUCCESS) && 
    ...
    StreamMgr_print_error(streams,  "Invalid value for the bmc_loopback variable.\n");
  }
  return false;
}
