...
#include "cudd/util.h"
#include "nusmv/core/utils/list.h"         /* Self declaration        */
...
lsList lsSingleton(data, itemHandle)
 ...
 */
{
    ...
    newList->topPtr = newList->botPtr = newElem;
    newList->length = 1;
...
static inline void lsDestroyNN(lsGeneric list)
/*
 * Frees all resources associated with the specified list. Used by
 * lsDestroyList and lsDestroyListList.
 */
{
    lsDesc *realList;
    lsElem *index, *temp;
 ...
 */
{
    ...
    if (!list) return(LS_OK);
    realList = (lsDesc *) list;
    if (!delFunc) {
        lsDestroyNN((lsGeneric)list);
        return(LS_OK);
    }
    /* Get rid of elements */
    index = realList->topPtr;
...
/* Function to destroy a list of lists */
lsStatus lsDestroyListList(lsList list) {
    return lsDestroy(list, lsDestroyNN);
}
...
static inline lsGeneric lsCopyNN(lsList list)
/*
 * Returns a copy of list `list'.  Used by lsCopyList and lsCopyList.
 */
{
    lsList newList;
    lsGen gen;
    ...
    while (lsNext(gen, &data, LS_NH) == LS_OK) {
      (void) lsNewEnd(newList, data, LS_NH);
    }
    lsFinish(gen);
    return (lsGeneric)newList;
}
 ...
 */
{
    ...
    if (!copyFunc) return (lsList)lsCopyNN(list);
    newList = lsCreate();
    gen = lsStart(list);
    while (lsNext(gen, &data, LS_NH) == LS_OK) {
...
lsList
lsCopyListList(lsList list)
{
  return lsCopy(list, (LS_PFLSG)lsCopyNN);
}



static inline lsList lsAppendCopy(lsList list1, lsList list2)
{
    lsList newList;
    lsGen gen;
    lsGeneric data;
    ...
    gen = lsStart(list1);
    while (lsNext(gen, &data, LS_NH) == LS_OK) {
        (void) lsNewEnd(newList, lsCopyNN((lsList) data), LS_NH);
    }
    lsFinish(gen);
    gen = lsStart(list2);
    while (lsNext(gen, &data, LS_NH) == LS_OK) {
...
static inline lsList lsAppendNN(lsList list1, lsList list2)
{
    lsList newList;
    lsGen gen;
    lsGeneric data;
...
lsList lsAppend(list1, list2, copyFunc)
lsList list1, list2;            /* Lists to be joined        */
lsGeneric (*copyFunc)(lsGeneric);       /* Routine to copy user data */
{
    lsList newList;
    lsGen gen;
    ...
    if (copyFunc == LS_COPY) return lsAppendCopy(list1, list2);
    if (!copyFunc) return lsAppendNN(list1, list2);
    newList = lsCreate();
    gen = lsStart(list1);
    while (lsNext(gen, &data, LS_NH) == LS_OK) {
        (void) lsNewEnd(newList, (*copyFunc)(data), LS_NH);
...
void lsJoin(list1, list2, copyFunc)
lsList list1, list2;            /* Lists to be joined        */
lsGeneric (*copyFunc)(lsGeneric);       /* Routine to copy user data */
{
    lsGen gen;
    ...
    if (!copyFunc)
      while (lsNext(gen, &data, LS_NH) == LS_OK) {
        (void) lsNewEnd(list1, data, LS_NH);
      }
    else
      while (lsNext(gen, &data, LS_NH) == LS_OK) {
        (void) lsNewEnd(list1, (*copyFunc)(data), LS_NH);
      }
    lsFinish(gen);
    return;
}
 ...
 */
{
    ...
    if (realGen->beforeSpot == NIL(lsElem)) {
        ...
        realGen->beforeSpot = realGen->mainList->botPtr;
        return LS_OK;
    } else {
 ...
 */
{
    ...
    if (realGen->beforeSpot == NIL(lsElem)) {
        /* Item added to the beginning of the list */
        (void) lsNewBegin((lsList) realGen->mainList, data, itemHandle);
        realGen->afterSpot = realGen->mainList->topPtr;
        return LS_OK;
    } else if (realGen->afterSpot == NIL(lsElem)) {
        /* Item added to the end of the list */
        (void) lsNewEnd((lsList) realGen->mainList, data, itemHandle);
...
lsStatus (*gen_func)(lsGen, lsGeneric*, lsHandle*);             /* Generator function to use      */
...
/* Merge sort by R. Rudell */
#include "nusmv/core/utils/lsort.h"

lsStatus lsSort(list, compare)
lsList list;                    /* List to sort        */
int (*compare)(char*, char*);           /* Comparison function */
/*
...
int (*compare)(char*, char*);           /* Item comparison function       */
