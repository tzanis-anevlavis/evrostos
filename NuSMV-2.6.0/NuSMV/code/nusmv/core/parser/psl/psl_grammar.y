...
| Expression TKINCONTEXT hierarchical_identifier
  { $$ = psl_expr_make_context(__psl_parser_env__, $3, $1); }
...
{
  $$ = psl_expr_make_replicated_property(__psl_parser_env__, 
                                         psl_expr_make_replicator(__psl_parser_env__, $1, psl_expr_make_atom(__psl_parser_env__, $2), $3, $5), $7);
...
ValueSet :
TKLC ValueRange_List ValueRange TKRC { $$ = psl_expr_make_cons(__psl_parser_env__, $3, $2); }
| TKLC ValueRange_List error TKRC { psl_error(); }
| TKBOOLEAN { $$ = psl_expr_make_boolean_type(__psl_parser_env__); }
;

ValueRange_List :
ValueRange_List ValueRange TKCOMMA { $$ = psl_expr_make_cons(__psl_parser_env__, $2, $1); }
...
TKX Expression                        { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKXBANG Expression                  { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKF Expression                      { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKG Expression                      { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKLB Expression TKU Expression TKRB { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $2, $3, $4); }
| TKLB Expression TKW Expression TKRB { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $2, $3, $4); }

/* Simple Temporal Operators */

| TKALWAYS Expression            { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKNEVER  Expression            { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKNEXT   Expression            { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKNEXTBANG Expression          { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKEVENTUALLYBANG Expression    { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }


| Expression TKUNTILBANG Expression   { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKUNTIL Expression       { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKUNTILBANG_ Expression  { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKUNTIL_ Expression      { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $1, $2, $3); }

| Expression TKBEFOREBANG Expression  { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKBEFORE Expression      { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKBEFOREBANG_ Expression { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKBEFORE_ Expression     { PSL_EXPR_MAKE_F_F2F_OP(__psl_parser_env__, $$, $1, $2, $3); }

 /* Extended Next (Event) Operators */
| TKX TKLB Expression TKRB TKLP Expression TKRP
{ PSL_EXPR_MAKE_EXT_NEXT_OP_WHEN(__psl_parser_env__, $$, $1, $6, $3); }
...
{ PSL_EXPR_MAKE_EXT_NEXT_OP_BOOL(__psl_parser_env__, $$, $1, $6, $3); }
...
{ PSL_EXPR_MAKE_EXT_NEXT_OP_WHEN_BOOL(__psl_parser_env__, $$, $1, $9, $6, $3); }
...
{ $$ = psl_expr_make_suffix_implication_strong(__psl_parser_env__, $1, TKPIPEMINUSGT, $3); }
| Sequence TKPIPEMINUSGT Sequence TKBANG
{ $$ = psl_expr_make_suffix_implication_strong(__psl_parser_env__, $1, $2, $3); }
| Sequence TKPIPEMINUSGT Sequence
{ $$ = psl_expr_make_suffix_implication_weak(__psl_parser_env__, $1, $2, $3); }
| Sequence TKPIPEEQGT Sequence TKBANG
{ $$ = psl_expr_make_suffix_implication_strong(__psl_parser_env__, $1, $2, $3); }
| Sequence TKPIPEEQGT Sequence
{ $$ = psl_expr_make_suffix_implication_weak(__psl_parser_env__, $1, $2, $3); }

| TKALWAYS Sequence { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKNEVER  Sequence { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }
| TKEVENTUALLYBANG Sequence { PSL_EXPR_MAKE_F2F_OP(__psl_parser_env__, $$, $2, $1); }

| TKWITHINBANG TKLP Sequence_or_Expression TKCOMMA Expression TKRP Sequence
{ $$ = psl_expr_make_within(__psl_parser_env__, $1, $3, $5, $7); }
...
{ $$ = psl_expr_make_whilenot(__psl_parser_env__, $1, $3, $5); }
...
{ $$ = psl_expr_make_abort(__psl_parser_env__, $1, $3); }

| Expression TKLTMINUSGT Expression { PSL_EXPR_MAKE_BW_BW2BW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKMINUSGT Expression   { PSL_EXPR_MAKE_BW_BW2BW_OP(__psl_parser_env__, $$, $1, $2, $3); }
;
...
Sere :
Expression { $$ = psl_expr_make_sere(__psl_parser_env__, $1); }
| Sequence { $$ = psl_expr_make_sere(__psl_parser_env__, $1); }

/* Composition Operators */
| Sere TKSEMI Sere          { $$ = psl_expr_make_sere_concat(__psl_parser_env__, $1, $3); }
| Sequence TKCOLON Sequence { $$ = psl_expr_make_sere_fusion(__psl_parser_env__, $1, $3); }
| Sequence AndOrOp Sequence { $$ = psl_expr_make_sere_compound_binary_op(__psl_parser_env__, $1, $2, $3); }

/* RegExpr Qualifiers */
| Sere TKLBSPLAT Opt_Count TKRB { $$ = psl_expr_make_repeated_sere(__psl_parser_env__, $2, $1, $3); }
| TKLBSPLAT Opt_Count TKRB      { $$ = psl_expr_make_repeated_sere(__psl_parser_env__, $1,
                                           psl_expr_make_empty(), $2); }
| Sere TKLBPLUSRB               { $$ = psl_expr_make_repeated_sere(__psl_parser_env__, $2,
                                           $1, psl_expr_make_empty()); }
| TKLBPLUSRB                     { $$ = psl_expr_make_repeated_sere(__psl_parser_env__, $1,
                                           psl_expr_make_empty(),
                                           psl_expr_make_empty()); }

 | Expression TKLBEQ Count TKRB  { $$ = psl_expr_make_repeated_sere(__psl_parser_env__, $2, $1, $3); }
 | Expression TKLBMINUSGT Opt_Count TKRB
                                 { $$ = psl_expr_make_repeated_sere(__psl_parser_env__, $2, $1, $3); }
 ;
...
Count :
TKNUMBER        { $$ = psl_expr_make_number(__psl_parser_env__, $1); }
| TKBASENUMBER  { $$ = psl_expr_make_base_number(__psl_parser_env__, $1); }
| Range
| hierarchical_identifier
;

Range :
   LowBound TKCOLON HighBound  { $$ = psl_expr_make_range(__psl_parser_env__, $1, $3); }
 ;

LowBound :
TKNUMBER        { $$ = psl_expr_make_number(__psl_parser_env__, $1); }
| TKMINUS TKNUMBER { $$ = psl_expr_make_number(__psl_parser_env__, -$2); }
| TKBASENUMBER  { $$ = psl_expr_make_base_number(__psl_parser_env__, $1); }
| hierarchical_identifier
;
...
| TKMINUS TKNUMBER { $$ = psl_expr_make_number(__psl_parser_env__, -$2); }
| TKBASENUMBER  { $$ = psl_expr_make_base_number(__psl_parser_env__, $1); }
| TKINF         { $$ = psl_expr_make_inf(__psl_parser_env__); }
| hierarchical_identifier
;

OBE_Property :
/* Universal Operators */
TKAX Expression { $$ = psl_expr_make_obe_unary(__psl_parser_env__, $1, $2); }
| TKAG Expression { $$ = psl_expr_make_obe_unary(__psl_parser_env__, $1, $2); }
| TKAF Expression { $$ = psl_expr_make_obe_unary(__psl_parser_env__, $1, $2); }
| TKA TKLB Expression TKU Expression TKRB { $$ = psl_expr_make_obe_binary(__psl_parser_env__, $3, $1, $5); }

/* Existential Operators */
| TKEX Expression { $$ = psl_expr_make_obe_unary(__psl_parser_env__, $1, $2); }
| TKEG Expression { $$ = psl_expr_make_obe_unary(__psl_parser_env__, $1, $2); }
| TKEF Expression { $$ = psl_expr_make_obe_unary(__psl_parser_env__, $1, $2); }
| TKE TKLB Expression TKU Expression TKRB { $$ = psl_expr_make_obe_binary(__psl_parser_env__, $3, $1, $5); }
;
...
Unary_Expression :
TKPLUS Primary     { PSL_EXPR_MAKE_NW2NW_OP(__psl_parser_env__, $$, $2, $1); }
| TKMINUS Primary  { PSL_EXPR_MAKE_NW2NW_OP(__psl_parser_env__, $$, $2, $1); }
| TKBANG Primary   { PSL_EXPR_MAKE_BW2BW_OP(__psl_parser_env__, $$, $2, $1); }
| TKBOOL TKLP Expression TKRP  { PSL_EXPR_MAKE_W2B_OP(__psl_parser_env__, $$, $3, $1); }
| TKWORD1 TKLP Expression TKRP { PSL_EXPR_MAKE_B2W_OP(__psl_parser_env__, $$, $3, $1);  }
| TKSIGNED TKLP Expression TKRP { PSL_EXPR_MAKE_W2W_OP(__psl_parser_env__, $$, $3, $1); }
| TKUNSIGNED TKLP Expression TKRP { PSL_EXPR_MAKE_W2W_OP(__psl_parser_env__, $$, $3, $1); }
| TKEXTEND TKLP Expression TKCOMMA Primary TKRP
                               { PSL_EXPR_MAKE_W_N2W_OP(__psl_parser_env__, $$, $3, $1, $5); }
| TKWRESIZE TKLP Expression TKCOMMA Primary TKRP
                               { PSL_EXPR_MAKE_W_N2W_OP(__psl_parser_env__, $$, $3, $1, $5); }
;

Binary_Expression :
Expression TKPLUS Expression      { PSL_EXPR_MAKE_NW_NW2NW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKUNION Expression   { PSL_EXPR_MAKE_T_T2T_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKIN Expression      { PSL_EXPR_MAKE_NB_NB2B_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKMINUS Expression   { PSL_EXPR_MAKE_NW_NW2NW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKSPLAT Expression   { PSL_EXPR_MAKE_NW_NW2NW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKSLASH Expression   { PSL_EXPR_MAKE_NW_NW2NW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKPERCENT Expression { PSL_EXPR_MAKE_NW_NW2NW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKEQ Expression      { PSL_EXPR_MAKE_NBW_NBW2B_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKEQEQ Expression    { PSL_EXPR_MAKE_NBW_NBW2B_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKBANGEQ Expression  { PSL_EXPR_MAKE_NBW_NBW2B_OP(__psl_parser_env__, $$, $1, $2, $3); }

| Expression TKLT Expression       { PSL_EXPR_MAKE_NW_NW2B_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKLE Expression       { PSL_EXPR_MAKE_NW_NW2B_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKGT Expression       { PSL_EXPR_MAKE_NW_NW2B_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKGE Expression       { PSL_EXPR_MAKE_NW_NW2B_OP(__psl_parser_env__, $$, $1, $2, $3); }

| Expression TKAMPERSAND Expression { PSL_EXPR_MAKE_BW_BW2BW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKAMPERSANDAMPERSAND Expression { PSL_EXPR_MAKE_B_B2B_OP(__psl_parser_env__, $$, $1, TKAMPERSAND, $3); }

| Expression TKPIPE Expression      { PSL_EXPR_MAKE_BW_BW2BW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKPIPEPIPE Expression  { PSL_EXPR_MAKE_B_B2B_OP(__psl_parser_env__, $$, $1, TKPIPE, $3); }

| Expression TKCARET Expression     { PSL_EXPR_MAKE_B_B2B_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKXOR Expression       { PSL_EXPR_MAKE_BW_BW2BW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKXNOR Expression       { PSL_EXPR_MAKE_BW_BW2BW_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKLTLT Expression       { PSL_EXPR_MAKE_W_NW2W_OP(__psl_parser_env__, $$, $1, $2, $3); }
| Expression TKGTGT Expression       { PSL_EXPR_MAKE_W_NW2W_OP(__psl_parser_env__, $$, $1, $2, $3); }

/* PSL BIT SELECTION  */
| TKWSELECT TKLP Expression TKCOMMA Expression TKCOMMA Expression TKRP
                          { $$ = psl_expr_make_bit_selection(__psl_parser_env__, $3, $5, $7); }

| Expression TKWCONCATENATION Expression
                          { $$ = psl_expr_make_word_concatenation(__psl_parser_env__, $1, $3); }
;

Conditional_Expression :
Expression TKQUESTIONMARK Expression TKCOLON Expression
{ $$ = psl_expr_make_ite(__psl_parser_env__, $1, $3, $5); }
;
...
case_list :
/* empty */    { $$ = psl_expr_make_failure(__psl_parser_env__, "case conditions are not exhaustive",
                                            FAILURE_CASE_NOT_EXHAUSTIVE); }
| Expression TKCOLON Expression TKSEMI case_list { $$ = psl_expr_make_case(__psl_parser_env__, $1, $3, $5); }
;

boolean :
  TKTRUE     { $$ = psl_expr_make_boolean_value(__psl_parser_env__, 1); }
| TKFALSE    { $$ = psl_expr_make_boolean_value(__psl_parser_env__, 0); }
;

number :
TKBASENUMBER   { $$ = psl_expr_make_base_number(__psl_parser_env__, $1); }
| TKNUMBER     { $$ = psl_expr_make_number(__psl_parser_env__, $1); }
| TKREALNUMBER { $$ = psl_expr_make_real_number(__psl_parser_env__, $1); }
;

word_number:
TKUWCONST TKLP Expression TKCOMMA Expression TKRP
                    { PSL_EXPR_MAKE_N_N2W_OP(__psl_parser_env__, $$, $3, $1, $5); }
| TKSWCONST TKLP Expression TKCOMMA Expression TKRP
                    { PSL_EXPR_MAKE_N_N2W_OP(__psl_parser_env__, $$, $3, $1, $5); }
| TKWORDNUMBER { $$ = psl_expr_make_word_number(__psl_parser_env__, $1); }
;

word_operators :
TKWSIZEOF TKLP Expression TKRP { PSL_EXPR_MAKE_W2N_OP(__psl_parser_env__, $$, $3, $1); }
| TKWTOINT  TKLP Expression TKRP { PSL_EXPR_MAKE_W2N_OP(__psl_parser_env__, $$, $3, $1);  }
;
...
hierarchical_identifier :
TKATOM   { $$ = psl_expr_make_atom(__psl_parser_env__, $1); free($1); }

| hierarchical_identifier TKDOT TKATOM
  { $$ = psl_expr_make_id(__psl_parser_env__, $1, psl_expr_make_atom(__psl_parser_env__, $3)); free($3); }

| hierarchical_identifier TKDOT TKNUMBER
  { $$ = psl_expr_make_id(__psl_parser_env__, $1, psl_expr_make_number(__psl_parser_env__, $3)); }

| hierarchical_identifier TKLB Expression TKRB
  { $$ = psl_expr_make_id_array(__psl_parser_env__, $1, $3); }
;
...
Concatenation :
TKLC Expression_List TKRC { $$ = psl_expr_make_concatenation(__psl_parser_env__, $2); }
 ;

/* This will be built in reversed order */
Expression_List :
  Expression_List TKCOMMA Expression { $$ = psl_expr_make_cons(__psl_parser_env__, $3, $1); }
| Expression  { $$ = psl_expr_make_cons(__psl_parser_env__, $1, psl_expr_make_empty()); }
;

Multiple_Concatenation :
TKLC Expression TKLC Expression_List TKRC TKRC
{ $$ = psl_expr_make_multiple_concatenation(__psl_parser_env__, $2, $4); }
;
