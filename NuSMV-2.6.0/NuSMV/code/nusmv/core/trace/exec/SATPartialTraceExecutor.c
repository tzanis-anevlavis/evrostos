...
/*!
  ...
  \brief Implementation of class 'SATPartialTraceExecutor'
...
/*!
  \brief Partial trace execution method

  Performs partial trace re-execution. Algorithm is
  selected depending on the value of restart flag given at
  construction time

  \se a complete trace is built and returned in case of success
...
/*!
  \brief The SATPartialTraceExecutor class virtual finalizer
...
/*!
  ...
  Executes a trace on the fsm given at construction time
  ...
  \se a complete trace is built and returned is case of success
...
/*!
  ...
  The trace is executed using SAT, that is a proof that
  ...
  TraceMgr and its index is written into trace_index;
 ...
 const NodeList_ptr language, int* n_steps)
{
  ...
  if (SAT_SOLVER_UNSATISFIABLE_PROBLEM == satResult) {
    ...
    do {
      ...
      step = TraceIter_get_next(step);
      if (TRACE_END_ITER != step) {
        be_ptr be_input, be_comb, be_next;
...
/*!
  ...
  Private service of sat_partial_trace_executor_execute
