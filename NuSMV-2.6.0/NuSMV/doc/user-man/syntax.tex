...
\operator{abs}, \operator{max}, \operator{min}
\end{quote}
...
basic_expr ::
    ...
    | \operator{abs} \textbf{(} basic\_expr \textbf{)}                        -- absolute value
    ...
    | basic_expr \operator{..} basic_expr      -- pure integer set expression
...
Yet another way to write a set expression is to use the binary operator \operator{..}
(\code{<TWO DOTS>}). The two
...
This is just a shorthand for a set expression containing the list
of \grammar{integer numbers} included between the lower and the upper bound.
%
For example, \code{-1..5} and
...
ltl_expr ::
    ...
    | \reserved{G} bound ltl_expr       -- bounded globally
    | \reserved{F} ltl_expr             -- finally
    | \reserved{F} bound ltl_expr       -- bounded finally
    ...
    | \reserved{H} bound ltl_expr       -- bounded historically
    | \reserved{O} ltl_expr             -- once 
    | \reserved{O} bound ltl_expr       -- bounded once
...
\begin{itemize}
  ...
  \item \code{\reserved{H} [l,u] \textit{p}} is true at time $t$ if
        \textit{p} holds in \emph{all} previous time steps $t-u \leq t^\prime \leq t-l$.
  \item \code{\reserved{O} \textit{p}} is true at time $t$ if
        \textit{p} held in \emph{at least one}
        of the previous time steps $t^\prime \leq t$.
  \item \code{\reserved{O} [l,u] \textit{p}} is true at time $t$ if
