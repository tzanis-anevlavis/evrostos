...
\noindent
\code{\toolprompt \command{alias} read "read\_model -i \%:1.smv ; set input\_order\_file \%:1.ord"\\
   \toolprompt read short\\
}
  will create an alias `read', execute "read\_model -i short.smv; 
    set input\_order\_file short.ord".
  And again:\\
   \texttt{\toolprompt \command{alias} echo2 "echo Hi ; echo \%* !"\\ 
  ...
  dependency in the alias definition. e.g.\\
   \texttt{\toolprompt \command{alias} foo "echo print\_bdd\_stats; foo"
  }\\
